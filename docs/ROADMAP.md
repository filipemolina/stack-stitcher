...
| --- | --- | --- |
| 1 | `image-search.md` — **Phase 1 only** (`n` adds a service) | The biggest hole in the lifecycle, and the one every later plan is blocked on. `ApplyServiceFragment` can edit a service but not create one, so "add a service" means leaving the app. Phases 2–4 (Docker Hub search, tag picker) are the *feature*; Phase 1 is the *foundation*, and splitting it out is what unblocks 2, 4 and 11. |
| 2 | `service-aware-empty-state.md` | Small, and it belongs directly after step 1 rather than before it: once `n` can add a service, the first-run empty state can say something useful to a user with an empty file instead of explaining a concept they cannot act on yet. Doing it earlier means writing that copy twice. |
| 3 | `service-urls.md` — **Phase 1** | Two days, and the best demo material in this list: the panel showing `http://192.168.1.10:14533` as a hyperlink the terminal will open explains the whole premise in one frame. `chrome.PortLabel` already exists (landed with `group-table-legibility.md`). Phase 2 (reverse-proxy labels) can trail behind indefinitely. |
| 4 | `healthcheck-insertion.md` | ~2 days, and it completes a lifecycle stage the ask names explicitly. It depends on nothing from steps 1–2 in code, but it reads as a natural pair with them: adding a service and giving it a working probe are the same sitting for the user. |
| 5 | `docker-disk-usage.md` | A day and a half for an overlay that answers a question nothing else in the stack does — 70% of this machine's 60 GB of images is reclaimable and no tool says so. Not a page, so the *no statistics page* decision above stands. |
| 6 | `env-secrets.md` | The largest of the feature plans and the last lifecycle gap. Placed after the smaller ones so the pattern for "a new page with its own modals" is already established twice over — and so a long branch is not the thing blocking everything else. Has open owner decisions listed in its *Who decides* section; settle those before starting, not during. |
| 7 | `resources-page.md` — **Phases 0–1** | The read-only networks/volumes inspector. Last of the feature work because it is the least urgent and because its Phase 2 (writes) must wait for the write-safety story below. **Its Phase 0 is a one-line correctness fix and can be pulled forward at any time** — see the note under this table. |
| 8 | `cross-platform-testing.md` | Before the release work, not after: it costs $0 on GitHub's runners and it answers *what actually works on macOS and Windows*. Publishing binaries first and finding out afterwards is the wrong order. |
| 9 | `release-distribution.md` | Now the tool is worth installing, make it installable by people without a Go toolchain — which today is most of the audience. |
| 10 | `launch-and-outreach.md` | The announcement, and the gate: it opens with the lifecycle checklist that steps 1–6 close. |
| — | `adopt-unmanaged-containers.md` | **After the launch.** Phase 2 is blocked on step 1's insert primitive, and the feature finds nothing at all on a tidy machine — its audience is the homelab that grew by accretion, which is a good post-launch story rather than a gate. Phase 1 (see them, switch to their file, remove them) is standalone and can be pulled forward if it is wanted sooner. |
...
| — | `ai-service-authoring.md` | **Deliberately out of the sequence.** It is the only plan that adds a dependency on something outside the repo, its own Phase 1 (an offline catalog) delivers most of the value with none of that, and it needs step 1's insert path to exist first. Pick it up after the launch, or take its Phase 1 alone at any point. |
...
roughly six days to the road to step 10, and that trade was made knowingly.
`group-table-legibility.md` and `docker-preflight.md` are the first two of
the four to land — see *Done, and kept for the record* below.

Three items in that sequence are not plans and would otherwise fall through the
cracks:

- **Cut a `v0.1.0` tag early — before step 1, not at step 9.** Done, on
...
- **A write-safety story, before step 10.** The app rewrites the user's compose
...
as the last word. See *Docker's absence is a diagnosis, not an error* in
`docs/DESIGN.md`.
